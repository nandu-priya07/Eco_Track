      ...
      <div style={{
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <h2 style={{ fontSize: 'clamp(1.4rem, 3vw, 2rem)', fontWeight: 800, color: 'var(--color-text-main)', marginBottom: '0.75rem' }}>
            ...
            <p style={{ color: 'var(--color-text-muted-dark)', maxWidth: '600px', margin: '0 auto', fontSize: '0.95rem', lineHeight: 1.7 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '1rem' }}>
            {MISSION_STATS.map(({ value, label, icon }) => (
              <div key={label} style={{
                background: 'rgba(var(--color-eco-bg-rgb),0.6)',
              ...
              >
                ...
                <div style={{ fontSize: '0.78rem', color: 'var(--color-text-muted-dark)', marginTop: '0.15rem' }}>{label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 1.5rem 2rem' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '2.5rem', marginBottom: '3rem' }}>
          ...
          <div style={{ gridColumn: 'span 1' }}>
            ...
            <p style={{ fontSize: '0.875rem', lineHeight: 1.7, marginBottom: '1.25rem', color: 'var(--color-text-muted-dark)' }}>
            ...
            <div style={{ display: 'flex', gap: '0.6rem', marginBottom: '1.5rem' }}>
              {[
                ...
                <a key={label} href="#" title={label} style={{
                  width: 36, height: 36,
                  background: 'rgba(var(--color-eco-card-rgb),0.8)', border: '1px solid var(--color-border)',
          ...
          {/* Quick Links */}
          <div>
            <h4 style={{ color: 'var(--color-text-body)', fontWeight: 700, marginBottom: '1.25rem', fontSize: '0.875rem', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <span style={{ color: '#10b981' }}>⚡</span> Quick Links
            </h4>
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              {QUICK_LINKS.map(({ label, to }) => (
                <li key={label}>
                  <Link to={to} style={{ color: 'var(--color-text-muted-dark)', textDecoration: 'none', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '0.4rem', transition: 'color 0.2s, gap 0.2s' }}
                    onMouseEnter={e => { e.currentTarget.style.color = '#34d399'; e.currentTarget.style.gap = '0.6rem'; }}
                    onMouseLeave={e => { e.currentTarget.style.color = 'var(--color-text-muted-dark)'; e.currentTarget.style.gap = '0.4rem'; }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
              {[
                ...
                <div key={title} style={{ display: 'flex', gap: '0.6rem', alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{title}</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--color-text-muted-dark)', whiteSpace: 'pre-line', lineHeight: 1.5 }}>{detail}</div>
                  </div>
                </div>
              ))}
            </div>
              ...
              {[
                ...
                <div key={label} style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.76rem', color: 'var(--color-border-light)' }}>{desc}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{ fontWeight: 700, color: 'var(--color-text-body)', marginBottom: '0.25rem' }}>Our Sustainability Pledge</div>
            <p style={{ fontSize: '0.85rem', color: 'var(--color-text-muted-dark)', lineHeight: 1.6 }}>
        ...
        {/* Bottom Bar */}
        <div style={{
          borderTop: '1px solid rgba(var(--color-eco-card-rgb),0.8)',
          paddingTop: '1.5rem',
          display: 'flex', justifyContent: 'space-between',
          flexWrap: 'wrap', gap: '0.75rem', fontSize: '0.82rem',
          alignItems: 'center',
        }}>
          <span style={{ color: 'var(--color-border-light)' }}>© {new Date().getFullYear()} EcoTrack® — All rights reserved.</span>
          <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
            {['Privacy Policy', 'Terms of Service', 'Cookie Policy', 'Accessibility'].map(item => (
              <a key={item} href="#" style={{ color: 'var(--color-border-light)', textDecoration: 'none', transition: 'color 0.2s' }}
                onMouseEnter={e => e.target.style.color = '#34d399'}
                onMouseLeave={e => e.target.style.color = 'var(--color-border-light)'}>
                {item}
              </a>
            ))}
          </div>
